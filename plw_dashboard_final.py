...
for bar in bars:
    ax.text(bar.get_width() + 25, bar.get_y() + bar.get_height()/2, f"{int(bar.get_width()):,}",
            ha="right", va="center", color="black", fontsize=9)
st.pyplot(fig)

# --- Camp Site Performance (PLW Wise) ---
st.subheader("📈 Camp Site Performance (PLW Wise)")
...
for bars in [bar1, bar2]:
    for bar in bars:
        ...
        ax.text(bar.get_x() + bar.get_width()/2, label_height - (0.03 * label_height),  # slightly below top
